...
(defn- cmd-index [args]
  (when (or (= (first args) "--help") (= (first args) "-h"))
    ...
    (println "  --es-url <url>       Elasticsearch URL (default: $GLITCH_ES_URL or localhost:9200)")
    ...
    (println "  --depth <n>          BFS depth for edge traversal (default: 1)")
    (println "  --context <name>     Full context for a symbol (definition + edges)")
    (println)
