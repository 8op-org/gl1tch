...
`;const a=s==="arm64"?"amd64":"arm64",l=e==="darwin"?a==="arm64"?"Apple Silicon":"Intel":a==="arm64"?"ARM64":"x86_64",d=t.assets[`${e}_${a}`]??"";d&&(c+=`Wrong arch? ${l}: ${d}`)}$(c)}let g=null,S=0,L=[];async function yt(){let t=[];try{t=await(await fetch("/screenshots/index.json")).json()}catch{F("output-error","Could not load screenshots. Run tools/screenshots/take.sh first.");return}if(t.length===0){F("output-dim","No screenshots yet. Run: tools/screenshots/take.sh");return}L=t,S=0,g&&g.remove(),g=document.createElement("div"),g.className="output-line screenshot-gallery",g.setAttribute("tabindex","0"),j.appendChild(g),Y(),Z(),g.addEventListener("keydown",e=>{e.key==="ArrowRight"||e.key==="l"?(e.preventDefault(),S=(S+1)%L.length,Z()):e.key==="ArrowLeft"||e.key==="h"?(e.preventDefault(),S=(S-1+L.length)%L.length,Z()):(e.key==="Escape"||e.key==="q")&&(e.preventDefault(),Et(),E.focus())}),g.focus()}function Z(){if(!g||L.length===0)return;const t=L[S];for(;g.firstChild;)g.removeChild(g.firstChild);const e=document.createElement("div");e.className="sg-header";const s=document.createElement("span");s.className="sg-counter",s.textContent=`[${S+1}/${L.length}]`;const n=document.createElement("span");n.className="sg-caption",n.textContent=t.caption;const r=document.createElement("span");r.className="sg-keys",r.textContent="← → navigate · esc close",e.appendChild(s),e.appendChild(n),e.appendChild(r);const i=document.createElement("div");i.className="sg-img-wrap";const o=document.createElement("img");o.className="sg-img",o.setAttribute("loading","lazy"),o.src=`/screenshots/${t.file.replace(/[^a-zA-Z0-9._-]/g,"")}`,o.alt=t.caption,i.appendChild(o);const c=document.createElement("div");c.className="sg-dots",L.forEach((u,a)=>{const l=document.createElement("span");l.className=a===S?"sg-dot sg-dot-active":"sg-dot",c.appendChild(l)}),g.appendChild(e),g.appendChild(i),g.appendChild(c)}function Et(){g&&(g.remove(),g=null)}const me=[{name:"code-review",desc:"local model reads the diff, cloud model judges it",yaml:`name: code-review
version: "1"
...
steps:
  ...
  - id: draft
    ...
    prompt: |
      ...
      Group into: Added, Changed, Fixed, Removed.`}];function Ct(){const t=me[Math.floor(Math.random()*me.length)];$(`${t.name}  —  ${t.desc}

${t.yaml}

run it:  glitch pipeline run ${t.name}.yaml
type /pipelines again for another example.`)}function wt(){const t=document.getElementById("terminal-workspace"),e=document.getElementById("docs-panel");!t||!e||(t.classList.add("docs-open"),e.removeAttribute("aria-hidden"),F("output-dim",">> docs panel open  ·  /docs to close  ·  × to dismiss"))}function oe(){const t=document.getElementById("terminal-workspace"),e=document.getElementById("docs-panel");!t||!e||(t.classList.remove("docs-open"),e.setAttribute("aria-hidden","true"),E.focus())}function F(t,e){const s=document.createElement("div");s.className=`output-line ${t}`,s.textContent=e,j.appendChild(s),Y()}let G=null;function ge(t){return new Promise(e=>setTimeout(e,t))}function $(t){G&&(clearTimeout(G),G=null);const e=document.createElement("div");return e.className="output-line output-response",j.appendChild(e),Y(),new Promise(s=>{let n=0;function r(){if(n<t.length){e.textContent=t.slice(0,n+1),n++;const i=t[n-1];G=setTimeout(r,i===`
`?25:i==="."||i===":"?30:10),Y()}else s()}r()})}async function Bt(){await ge(400),await $("hey. your AI workspace lives in your terminal. local models run by default — nothing leaves your machine, no bill unless you ask for cloud."),await ge(350),await $("type /install to get it running. /pipelines to see what it can do. /docs if you want the full picture.")}function Fe(){Array.from(j.children).slice(2).forEach(t=>t.remove())}function Mt(){if(!_||!E.value.startsWith("/"))return;const t=E.value.slice(1).toLowerCase(),s=[...Object.keys(_.commands),"screenshots","clear","docs"].filter(n=>n.startsWith(t));s.length===1?E.value=`/${s[0]}`:s.length>1&&F("output-dim",s.map(n=>`/${n}`).join("  "))}function Y(){j.scrollTop=j.scrollHeight}function xt(){const t=document.getElementById("hexbg");if(!t)return;const e=t.getContext("2d"),s=28,n=18;let r,i,o,c,u;function a(){return Math.floor(Math.random()*256).toString(16).padStart(2,"0").toUpperCase()}function l(){t.width=window.innerWidth,t.height=window.innerHeight}function d(){r=Math.ceil(t.width/s)+2,i=Math.ceil(t.height/n)+3,o=Array.from({length:r},()=>Math.random()*i*n),c=Array.from({length:r},()=>.25+Math.random()*.45);const f=i*4;u=Array.from({length:f},()=>Array.from({length:r},a))}function p(){e.clearRect(0,0,t.width,t.height),e.font='12px "JetBrains Mono", monospace',e.fillStyle="rgba(135, 135, 175, 0.07)";for(let f=0;f<r;f++){o[f]=(o[f]+c[f])%(u.length*n);const m=Math.floor(o[f]/n),y=o[f]%n;for(let A=0;A<=i+1;A++){const x=A*n-y,B=(m+A)%u.length;e.fillText(u[B][f],f*s,x)}}requestAnimationFrame(p)}l(),d(),window.addEventListener("resize",()=>{l(),d()}),p()}document.addEventListener("DOMContentLoaded",()=>{xt(),ft()});
