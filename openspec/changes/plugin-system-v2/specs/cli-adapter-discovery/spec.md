...
### Requirement: Hardcoded CLI tool registry drives binary autodetection
**Reason**: The `knownCLITools` hardcoded slice in `internal/discovery` is replaced by provider capability profiles. Binary detection is now driven entirely by `binary` fields in provider profile YAML files (bundled and user-installed). Adding a new provider no longer requires modifying Go source.
**Migration**: Add a YAML profile to `~/.config/orcai/providers/<name>.yaml` with a `binary` field. Orcai will autodetect the binary via `exec.LookPath` using that field. No code change required.
