...
Each output line SHALL be processed through the feedline parser pipeline before rendering. If a parser matches the line, its display output replaces the raw word-wrapped line. If no parser matches, the line renders as before (indented, dimmed, word-wrapped to panel width).
...
#### Scenario: JSON step output renders collapsed
- **WHEN** a step output line is valid JSON
- **THEN** the line renders as a collapsed JSON summary row via the feedline parser pipeline
