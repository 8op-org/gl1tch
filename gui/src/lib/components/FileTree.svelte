...
<ul class="tree" style="--depth:{depth}">
  {#each entries as entry}
    {#if entry.isDir}
      <li>
        <button class="tree-item dir" onclick={async () => { toggle(entry.name); onselect?.(entry.path); if (entry.loadChildren && !entry.children) { await entry.loadChildren(); expanded = {...expanded}; } }}>
