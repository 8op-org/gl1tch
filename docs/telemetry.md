...
Common step span attributes:

- `step.status` — `"ok"` or `"error"`
- `step.duration_ms` — elapsed time
- For LLM steps: `llm.model`, `llm.provider`, `llm.prompt_tokens`, `llm.completion_tokens`, `llm.temperature`
- For shell steps: `shell.command`, `shell.exit_code`, `shell.output_length`
...
Edit `internal/telemetry/telemetry.go` and change line 83 from:
